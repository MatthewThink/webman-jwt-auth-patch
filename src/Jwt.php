...
class Jwt
{
    ...
    public function validate(string $token): array
    {
        ...
        if ($this->manager->getBlacklistEnabled() && $this->auth->blackList->hasTokenBlack($claims, $this->config)) {
            throw new HasLoggedException('The token is in blacklist', 401);
        }
        ...
        if (Utils::isPast($claims->get('exp')->getTimestamp(), $leeway)) {
            if ($this->config->getAutoRefresh()) {
                if (!($ttl = $this->config->getRefreshTTL())) $this->automaticRenewalToken();

                if (Utils::isPast($claims->get('iat')->getTimestamp() + $ttl, $leeway))
    ...
    public function automaticRenewalToken() {
        ...
        [$token, $refresh_at] = [strrev($this->make(end($jti), $claims)->toString()), $this->config->getRefreshTTL()];
        try {
            $response = is_array($tips = $this->manager->getAutomaticTips()) ? \json($tips) : \response($tips);
            $response->withHeaders([
                'Access-Control-Expose-Headers'     => 'Automatic-Renewal-Token,Automatic-Renewal-Token-RefreshAt',
                'Automatic-Renewal-Token'           => $token,
                'Automatic-Renewal-Token-RefreshAt' => $refresh_at ?: INF
            ]);

            \request()->connection->send($response);
        } catch (\Exception $e) {
            throw new TokenContinueException($token, $refresh_at);
        }
    }
